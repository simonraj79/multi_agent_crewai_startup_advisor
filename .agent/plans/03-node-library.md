...
1. `python -c "import typing; from brief_crew.builder.document import NodeKind; print(len(typing.get_args(NodeKind)))"` prints `10`; `types/builder.ts` `NodeKind` has the same ten; ~~`tests/builder/test_client_fixtures.py` byte-compares the kind list~~ — **amended 2026-09-04, S9 ruling: `frontend/tests/nodeKinds.spec.ts` reads `NodeKind` and `_OUT_PORTS_BY_KIND` out of `document.py` AT RUN TIME and asserts equality, which proves the same property with no snapshot in between. A generated fixture would prove only that the snapshot matches the generator today, and adding one on top would be a SECOND mirror of one truth — which is section 14's defect 2 committed again, where `PROBLEM_CODES` agreed with itself at 27 against a server emitting 30. `test_client_fixtures.py` keeps the two fixtures it owns, `builderBackEdges.json` and `builderProblemCodes.json`, where a snapshot is the only available mechanism.**
