# Langfuse figures - run `ca13fc73-0ca9-4a31-bc8c-6e53ed1d562d`

| field | value |
| --- | --- |
| session found | yes |
| traces | 1 |
| trace names | ug_fd12e0a6 |
| environment | live |
| userId | proof-runner |
| tags | gates:auto, mode:run, ug_fd12e0a6 |
| trace output.status | failed |
| observations | 32 |
| observation types | AGENT:1, EVENT:19, GENERATION:6, SPAN:6 |
| observation roles | agent:1, generation:6, node:23, run:1, task:1 |
| open observations | 19 |
| scores | 3 |
| wall clock (s) | 0.751 |

## Totals

| metric | value |
| --- | --- |
| generations | 6 |
| input tokens | 0 |
| output tokens | 0 |
| total tokens | 0 |
| cost | $0.000000 |
| generations with no cost | 6 |
| tool observations | 0 |
| generation ids present | 0 |
| generations with no id | 6 |
| DUPLICATE generation ids (E1) | 0 |

## Ingestion visibility (measured by polling, not assumed)

| field | value |
| --- | --- |
| polls | 3 |
| rate-limited polls (429) | 0 |
| other poll errors | 0 |
| first observation visible after (s, from poll start) | 2.199 |
| count stable after (s, from poll start) | 8.676 |
| first visible after the run's terminal frame (s) | 91.073 |
| stable after the run's terminal frame (s) | 97.073 |
| stable within the timeout | yes |
| observation count at that point | 32 |
| stability window (s) | 5.000 |
| timeout (s) | 120.000 |
