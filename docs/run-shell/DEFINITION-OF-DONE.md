...
| T2.8 | No dropped frames at 119+ events. | `evidence/T2/perf.json`: a Playwright replay of ≥ 119 frames at live pace measuring `requestAnimationFrame` intervals; count of intervals > 34 ms (two missed 60 Hz frames) must be 0 during the replay window, and p95 ≤ 20 ms. The script is committed as `e2e/cast-perf.spec.ts`. | RV |
