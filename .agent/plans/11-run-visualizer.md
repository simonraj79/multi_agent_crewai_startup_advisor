...
**P-09 — one easing curve, and a third opacity nobody declared.**

*The curve.* The active card ran
`node-glowing 4s linear infinite, node-pulse 2s ease-in-out infinite` where
`docs/chatdev-notes.md` §2 records the reference (`vueflow.css:81-125`) as
`node-pulse 2s ease-out`. Names, both periods and the dual-period intent all
matched; the second timing function did not. Rubric 8 is *"the reference's
curves and periods, quoted in `docs/chatdev-notes.md` §2"*, so this is a
criterion rather than a preference — and the departure was **undeclared**,
which is the part that made it a defect rather than a decision.

`run-canvas.spec.ts`'s animation audit asserted animation *names* and passed
throughout, which is the more interesting half: a name is the part of an
animation a regression is least likely to touch. `animationTimings` now reads
the period and the curve as well, from `getComputedStyle` rather than from
`getAnimations()` — whose `effect.getTiming().easing` is the KEYFRAME easing and
answers `linear` whatever the shorthand said.

*The opacity.* Criterion 3 names two levels on a card — receded 0.55, present
1.0 — and a live run measured **three**: `{0.55, 0.6, 1.0}`. The third was
`.workflow-node.is-quiet`, the empty quarantine card, carrying an `opacity: 0.6`
of its own. On the canvas that reads as a fourth node state. It takes
`--recede-opacity` now, because an empty quarantine node *is* a card that is
stepped back and that token already means exactly that.

The E2E could not have caught it: it named two cards and checked each, which
cannot show that there are only two. `choreography.spec.ts` now collects the
distinct opacity of **every** `.workflow-node` mid-run and asserts the set is
`[0.55, 1]`.

> **A correction to the entry above this one.** The P-08 record was first
> appended to `.agent/plans/11-visualizer.md`, a file that did not exist —
> `>>` created it — and it was committed in `feee876` before the mistake was
> noticed. The stray file is deleted and its content is here, in the plan that
> actually owns this surface. Recorded rather than quietly fixed, because the
> failure mode is worth naming: a redirect append never reports a wrong path.
