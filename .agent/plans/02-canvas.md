...
8. **Rubric 6:** `e2e/builder-perf.spec.ts` loads `frontend/tests/fixtures/perf48.json` (24 flow + 24 attachment nodes, the bound maximum), scripts 3 s of wheel zoom and 3 s of drag pan, samples `requestAnimationFrame` deltas, and asserts ~~**mean ≤ 16.7 ms**~~ and **p95 ≤ 20 ms** on the `chromium` project — **amended 2026-09-04, S9 ruling, on this spec's own measurements: the mean clause measured the HARNESS, not the canvas.** A 60Hz display floors the mean at 16.667ms, so 16.7 allows 0.033ms/frame — over ~370 frames, less than ONE dropped frame. Measured: the same gesture reads 16.849 at **one** node and 16.846 at 48 and at 60, three numbers within 0.003ms across a sixtyfold difference in canvas work, while an idle 48-node canvas reads 16.666 with no late frame at all. The overrun is CDP-driven input on the paint thread and is identical at every graph size. Replaced by two clauses that measure what rubric 6 asks: **the mean at N nodes may exceed the same gesture at ONE node by at most 0.5 ms/frame** (immune to the harness, since both sides pay it; margin today −0.003ms), **and an absolute mean ≤ 17.0 ms** so the amendment cannot hide a real regression — a canvas costing 1ms a frame would read 17.85 and fail it. **And the removed clause was NONDETERMINISTIC**, which is the stronger reason: two runs of the same code forty minutes apart read 16.846 (fail, 2–3 dropped frames) and 16.666 (pass, none). A flaky performance gate teaches a team to re-run until green and then to ignore it. The scaling clause measured **+0.0000 ms at 48 nodes and +0.0004 ms at 60** on the very run that produced those figures; the numbers are written to `benchmarks/perf/canvas.json`. A client-only 60-node render (no server) passes the same budget.
