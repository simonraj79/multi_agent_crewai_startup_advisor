...
| Event | Carries |
|---|---|
| `LLMCallCompletedEvent` | `usage`, `model`, `response_id`, plus `task_id`/`task_name`/`agent_id`/`agent_role` from the base class |
...
the call being observed. **Handler exceptions are caught and logged, never
