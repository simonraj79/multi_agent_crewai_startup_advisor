...
Last reconciled: **2026-08-30 (fourth pass)**, against source, executable tests
re-run locally, a **live deployment**, and — new this pass — a **green CI run**.
Every row below names the test or the source path it rests on; a row with
neither is not evidence.

Nothing in this pass moved the three things that actually gate the product: the
paid acceptance run, the fan-out measurement, and the rubric review's verdict.
Deployment and a green pipeline move none of them, and caveat 6 below explains
why the rubric one now points the other way.
...
| `tests/service/` | 133 (`test_app` 5, `test_cors` 16, `test_gate_expiry` 12, `test_gate_fields` 17, `test_gate_resume_race` 11, `test_graph_registry` 6, `test_observability` 19, `test_persistence` 5, **`test_run_admission` 37**, `test_serve_env` 5) |
...
**7. CI is green for the first time, and the defect it exposed was an
environment assumption, not a test.** As of `e539811` both jobs pass on
`ubuntu-latest` (`Ran 415 tests in 15.823s`); the three commits before it all
