...
## The prompt-fix runs — the two defects the OUTPUT had, 2026-09-05

Defect 4 above, and the reasoning leak the `max_iter` verification run left
behind. Both are defects of the **deliverable** rather than of the runtime:
every run involved `completed`, billed, and satisfied every assertion
`e2e/templates.spec.ts` makes. Both were repaired in the templates' own
**prompts** — `frontend/src/data/templates/{sequentialPipeline,newsToSocial}.ts`
— and each repair was verified by one paid run.

Same backend shape as every section above, on a **different port** because
another agent held 8097: `PORT=8096`, no `SYNTHETIC`, `GET /docs` → **404**,
`PYTHONPATH=D:\MultiAgentSystem-wt\s1-01-ui\src`,
`BUILDER_ALLOW_GATELESS_GRAPHS=1`, `BUILDER_PLATFORM_FIRECRAWL_DEFAULT=1`,
`RUN_RATE_LIMIT_MAX_RUNS=100`, `CREWAI_TRACING_ENABLED=false`. Both documents
were created from the **committed fixture** (`documents.json`, regenerated by
`dump-templates.mjs` + `emit_builder_fixtures.py` after the prompt edit), so
what ran is what ships. Both took their saved sample from
`data/templates/testInputs.ts` — the same inputs the earlier runs used, so the
two pairs are comparable.

| template | run id | doc | status | elapsed | prompt / completion | calls | `cost_usd` | static | measured / static | gates |
| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `sequential-pipeline` | `412d6dfb` | `ug_eda92486` | **completed** | 34.7 s | 3,957 / 3,685 | 5 | $0.0142067 | $0.8416788 | **1.69 %** | 1 approve |
| `news-to-social` | `a85df3b9` | `ug_57f156d3` | **completed** | 33.2 s | 3,072 / 4,403 | 3 | $0.0173274 | $0.6103350 | **2.84 %** | 0 (ungated by design) |
| **two runs, summed** | | | | | **7,029 / 8,088** | **8** | **$0.0315341** | **$1.4520138** | **2.17 %** | |

Per node: `sequential-pipeline` — `research` `gemini-3.5-flash-lite:nitro`
3 calls $0.0015541, `analyse` same model 1 call $0.0018091, `write`
`gemini-3.8-flash` 1 call $0.0108435. `news-to-social` — `research`
`gemini-3.5-flash-lite:nitro` 2 calls $0.0015886, `write` `gemini-3.8-flash`
1 call $0.0157387. Evidence:
[`live/2026-09-05-sequential-pipeline-prompts.json`](live/2026-09-05-sequential-pipeline-prompts.json)
and
[`live/2026-09-05-news-to-social-prompts.json`](live/2026-09-05-news-to-social-prompts.json),
each carrying the whole frame stream, the researcher's and analyst's verbatim
utterances, the run's own usage row and its check block.

### Spend — and this delta is an UPPER BOUND, not a measurement

| | |
| --- | --- |
| balance before | **$27.290491885** (`total_usage` 92.709508115) |
| balance after | **$27.254979235** (`total_usage` 92.745020765) |
| balance delta | **$0.035513** |
| sum of the two `cost_usd` rows | **$0.031534** |
| difference | **$0.003979** |

**The difference is 12.6 %, and the honest reading is that the meter was not
mine alone.** Another agent's paid backend was listening on **8097** for the
whole of this session — confirmed by `Get-NetTCPConnection -LocalPort 8097`
after the last run — and `total_usage` had already moved **$0.054750** between
MISSION §7's 2026-09-05 reading (92.654757565) and this section's "before"
reading, with none of that spend mine. A shared OpenRouter account cannot
attribute a balance delta to one process, so **$0.031534 by the service is the
figure to carry**, and the delta above is recorded only because reading it is
the discipline every section on this page follows. It does not move the
`:nitro` question in either direction; the previous two sections' 1.0 % and
0.10 % were single samples on an unshared meter and this one is not a sample at
all.

### Defect 4 — CLOSED. The URLs now survive `research -> analyse -> write`.

Two sentences of prompt, one on each side of the join. The analyst's
`expected_output` gained:

> Then a final "## Sources" section listing, one per line, every URL from the
> research you relied on — verbatim, no shortening, and no URL that was not in
> the research.

and the writer's `description` gained:

> Cite only URLs that appear in the analysis's Sources section. Do not invent a
> URL and do not reconstruct one from memory. If a claim you want to make has
> no source in the analysis, make it anyway and mark it "(unsourced)".

**The check is mechanical and its ground truth is the RESEARCHER'S UTTERANCE**,
not a tool frame — for the reason this page already gives twice: a tool frame
carries a bounded `output_preview` and never the result rows, so URL-to-tool
closure is not answerable from a frame stream for any template. The utterance
is what the analyst was actually handed, and the evidence file carries it whole.

```text
URLs in the researcher's utterance         5
URLs in the analyst's "## Sources"         5   (0 not in the research)
URLs cited in the final body               5
URLs in the body's own "## Sources" list   5
fabricated relative to the analyst         0
dropped relative to the analyst            0
"(unsourced)" markers in the body         10
```

Run `eac19d14` on 2026-09-05, same template and same input, cited **zero** and
its sources section read *"the source analysis did not carry external URLs"*.

The ten `(unsourced)` markers are the repair working in the other direction and
are worth naming: the writer is now explicit about which of its recommendations
came from the evidence and which are its own. That is a change in what the
deliverable claims, not only in what it links.

**`task.output_schema` was considered and NOT used.** It is a flat
`dict[NodeId, ScalarType]` by design (`document.py`), so a `sources` field can
only be one string — no more mechanical than a `## Sources` heading — and
turning the analyst's output into a JSON object would change what
`${state.out__analyse}` renders into the writer's prompt from readable prose to
an escaped blob, on the one template whose whole job is being legible to
somebody who has never seen a flow. It would also add a "did the cheap tier
emit valid JSON" failure mode where there is currently none.

### The reasoning leak — CLOSED. The news post is the two sections and nothing else.

The `max_iter` verification run `d69c6986` returned **633 characters of the
writer's own working** as the entire `markdown_body`, opening mid-sentence and
ending mid-URL:

```text
thread-safe agents": Item 4 mentions "build thread-safe AI agents".

    Let's check the sentence count for Long:
    Sentence 1: Developers are tackling agent fragility and operational overhead …
```

`completed`, $0.0179, 4,249 completion tokens on the writer, and green under
every existing test. The repair is the writer's `expected_output`, which now
names the shape instead of describing it:

> Exactly two Markdown sections and nothing else: a line reading "# Short" with
> the post under it, then a line reading "# Long" with the long variant under
> it. Nothing before "# Short", nothing between the two sections but their own
> text, nothing after the long variant. No preamble, no commentary, no
> reasoning, no character counts.

with the matching negative in the `description`: *"Return the two sections and
nothing else. Do not introduce them, do not explain your choices and do not
show your working: no 'Here is', no 'Okay', no counting characters out loud,
no note about what you skipped."*

```text
starts with "# Short"                     true
"# Short" headings / "# Long" headings     1 / 1
Short section                            259 chars   (cap 280)
lines matching the leak patterns           0
hashtags                                   0          (cap 3)
URLs in the body                           3
URLs not in the researcher's utterance     0
items the researcher dated                 3 of 3
```

**No agent-config control was on that should have been off.** `planning` is
already `false` for every authored agent (`data/templates/authoring.ts`),
`planning_config` is null, `llm.reasoning_effort` is null, and `reasoning` /
`max_reasoning_attempts` are not fields at all — `AuthoredAgentConfig`'s own
docstring records that the 00 S9 ruling replaced them with `planning`. So the
leak was the model's thinking reaching `content`, and a prompt is the only
lever there was. Recorded because the obvious first guess was wrong and the
next reader will make it too.

### The 7-day window is now applied, and the researcher says so out loud

The `news-to-social` card's earlier verdict noted that *"the researcher's
prompt asks for 'the last 7 days', which HN Algolia's relevance search does not
honour — item 41984257 is from 2024."* The tool was never the problem: every
row it returns already carries `date` and `date_is_retrieval_time`
(`tools/hn_sentiment.py::_comment_date`), and **nothing in the prompt told the
agent to read either.** It does now:

> Every row the search returns carries a `date` and a `date_is_retrieval_time`
> flag. Drop every row whose `date_is_retrieval_time` is true — that date is
> the moment of the search, not the item.

with the writer told to *"skip any item that carries no date"*.

What the run produced is the honest outcome, and it is the one worth having:

```text
Fewer than three items fall inside the 7 days; here are the most recent ones found:

1. Redefining Human Expertise in Agentic Workflows
https://news.ycombinator.com/item?id=47115067
2026-02-22T21:47:24.000Z
…
```

**A hard drop was deliberately not written**, and the reason is a property of
the tool rather than a preference: HN Algolia's `search` endpoint is
relevance-ranked, not date-ranked, and the tool passes no numeric filter — so
"drop everything older than 7 days" would routinely leave the writer with
nothing and the template would ship a graph that returns an empty post. The
prompt instead requires the window to be **applied** and the ages to be
**visible**, and says so in one line when fewer than three items survive. Every
item now carries its date, which is what makes the writer's skip rule
enforceable at all. Read as a departure from a literal "drop", because it is
one.

### Both checks are now a test, and it fails on the old bodies

`tests/builder/test_paid_run_evidence.py` — **18 tests, no network, no model,
no credential.** It reads the two evidence files above and **recomputes** every
set from the utterance and the body rather than trusting the check block each
file carries; a missing evidence file **fails** rather than skipping. Proved
against the bodies that caused the defects rather than only against the ones
that fixed them:

```text
old news body    starts with "# Short": False;  leak patterns matched: 3 lines
old seq body     URLs in the body: 0
```

`LeakPatternTests` pins the regex itself against the four lines that actually
shipped, and against four lines of real deliverable that must not match — a
...
| --- | --- |
| `news-to-social` | **Did its job.** Four real Hacker News stories, four correct item URLs, both variants written to the prompt's shape. One liberty and one miss, below. **The miss — the ignored 7-day window — is closed by the prompt fix above**, and the LATER `max_iter` verification run of this template leaked the writer's own working as the whole deliverable, which is closed there too. |
| `sequential-pipeline` | **Did its job, minus its sources.** A 5,265-character brief with a headline and three sections, on topic and genuinely readable. But its sources list reads *"the source analysis did not carry external URLs"* — the URLs the researcher found do not survive `research -> analyse -> write`. **Re-run for money at `950205c` + the prompt fix and CLEAN** — 5 cited, 5 carried, 0 fabricated, 0 dropped; the prompt-fix section above. |
...
> **All four are now fixed, and a fifth is recorded above.** Defects 1-3 were
> closed on 2026-09-05 in `builder/runtime.py`, `builder/max_iter.py` and the
> gate contract; defect 4 and the writer's reasoning leak were closed the same
> day in the templates' PROMPTS, each verified by one paid run — the prompt-fix
> section above.
...
### 4 — `sequential-pipeline` loses its sources between the analyst and the writer. FIXED.

> **Fixed 2026-09-05 in the templates' own prompts**, and verified by paid run
> `412d6dfb`: 5 URLs in the researcher's utterance, 5 carried into the
> analyst's `## Sources`, 5 cited in the body, **0 fabricated and 0 dropped**.
> The analyst is now required to carry every URL it used into a Sources
> section verbatim, and the writer to cite only what that section holds and to
> mark an unsourced claim as one. The prompt-fix section above owns the
> sentences and the numbers.
