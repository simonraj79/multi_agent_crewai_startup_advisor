...
| 9 | Platform Firecrawl key as the default for every user, with a daily cap | 06 | yes, per-user override | **ON — ruled 2026-09-05.** `BUILDER_PLATFORM_FIRECRAWL_DEFAULT=1` set on the Render API service through the Render API that day, with `BUILDER_PLATFORM_FIRECRAWL_DAILY_CAP` at its default 50 per user per UTC day and the per-user override still honoured. Without it a cold sign-in cannot run any research template, which rubric 15 requires; the cap bounds the owner's exposure. **Measured limit, 2026-09-05:** the flag is read at exactly one site, `research_market_landscape`'s `credential_optional` (`builder/tools.py`); the three `firecrawl_*` catalogue entries stay `credential_optional=False` and `_firecrawl` raises without a credential, so they still need the author's own key and their cards say so. Widening the fallback to them is a plan-06 follow-up, not done - the shipped `news-to-social` template uses the keyless Hacker News search instead |
